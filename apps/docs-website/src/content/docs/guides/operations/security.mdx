...
Add HSTS at your TLS reverse proxy when the deployment uses HTTPS. Preserve the
frontend CSP header and the CSP meta element. A proxy can add more restrictive
directives, but these directives can stop multi-server connections or media.
...
| Web Push VAPID key pair (generated) | `RUNTIME_STATE` (`push_vapid_keys`) | Yes |
| Web Push VAPID key pair (operator-supplied) | `chatto.toml` / `CHATTO_PUSH_VAPID_PUBLIC_KEY`, `CHATTO_PUSH_VAPID_PRIVATE_KEY` | n/a |
