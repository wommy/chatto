...
Chatto retrieves that document and accepts only the exact callback registered
there. The user approves the identified client, and consent is remembered by
user and stable client ID:
...
OAuth callback trust is separate from CORS: Chatto accepts only an exact
callback from the validated client's CIMD document or a built-in registration
such as Chatto Desktop. See [Connections From Other Chatto Clients](/guides/operations/cross-server-connections/).
