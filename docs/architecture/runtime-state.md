...
| `push_vapid_keys` | JSON Web Push application-server (VAPID) key pair generated by the server when no operator key pair is configured, owned by [`core/push_vapid.go`](../../cli/internal/core/push_vapid.go). Startup creates the record, never overwrites it, so concurrent replicas settle on exactly one pair and existing browser subscriptions stay valid. It lives in `RUNTIME_STATE`, not `ENCRYPTION_KEYS`, so a normal backup keeps the pair together with the `push_subscription` records it authenticates. The record has no TTL. An operator-supplied key pair takes precedence and suppresses generation. |
