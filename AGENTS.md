...
- Do not give `git` an identity override. Do not pass `-c user.email=...` or
  `-c user.name=...`, and do not set `GIT_AUTHOR_EMAIL`. The clone is configured
  with the identity the session must use, and a commit made under a person's
  address is shown as Unverified and gives that person credit for the agent's
  work. The `pre-commit` hook in `.claude/hooks/git/` refuses such a commit.
